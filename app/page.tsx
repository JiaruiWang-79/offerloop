...
function Overview({ onNavigate }: { onNavigate: (step: Step) => void }) {
  ...
  return <div className="page-wrap dashboard-page">
    <div className="dashboard-head"><div><p className="eyebrow">2026 年 8 月 25 日</p><h1>求职进度</h1></div><div className="dashboard-actions"><button className="text-button">↥ 更新简历</button><button className="outline-button" onClick={() => onNavigate('jd')}>＋ 新建岗位</button></div></div>
    <div className="stats-strip"><StatCard value="13" label="已投递" tone="dark" /><StatCard value="4" label="待处理" /><StatCard value="2" label="面试中" /><StatCard value="0" label="Offer" /><StatCard value="0" label="已结束" /></div>
    <div className="dashboard-grid">
      <section className="schedule-card card"><div className="dashboard-section-title"><div><p className="eyebrow">APPLICATION SCHEDULE</p><h2>投递日程</h2></div><button className="month-button">2026 <span>⌄</span></button></div><div className="calendar-head"><button>‹</button><strong>8 月 2026</strong><button>›</button></div><div className="week-labels">{['一','二','三','四','五','六','日'].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarDays.map((day, index) => <span key={index} className={`${day < 1 || day > 31 ? 'muted-day' : ''} ${day === 25 ? 'today' : ''} ${[6, 12, 19, 26].includes(day) ? 'has-event' : ''}`}>{day > 0 && day < 32 ? day : ''}</span>)}</div><div className="schedule-list"><ScheduleItem date="今天 · 15:00" title="智行机器人 · AI 产品经理" type="面试" color="coral" /><ScheduleItem date="明天 · 截止" title="补充 AI 项目验证结果" type="准备" color="amber" /><ScheduleItem date="8 月 28 日" title="跟进 2 个已投递岗位" type="跟进" color="mint" /></div></section>
      <section className="analysis-card"><div className="analysis-card-head"><span>当前分析岗位</span><button onClick={() => onNavigate('decision')}>查看 →</button></div><h2>AI 产品经理实习生</h2><p>智行机器人 · 上海 / 混合办公</p><div className="analysis-score"><div><strong>68</strong><span>/ 100</span></div><small>证据覆盖度</small></div><div className="analysis-meta"><span>6 项能力要求</span><span>5 条经历证据</span></div><div className="analysis-links"><button onClick={() => onNavigate('jd')}>岗位拆解 <span>→</span></button><button onClick={() => onNavigate('evidence')}>证据匹配 <span>→</span></button></div></section>
    </div>
    <div className="recent-section"><div className="dashboard-section-title"><div><p className="eyebrow">RECENT APPLICATIONS</p><h2>最近投递</h2></div><button className="ghost-button" onClick={() => onNavigate('decision')}>查看全部 →</button></div><div className="recent-list"><RecentItem company="智行机器人" role="AI 产品经理实习生" date="今天" status="补充后投" /><RecentItem company="Qualcomm" role="AI Product Marketing Intern" date="8 月 23 日" status="已投递" /><RecentItem company="小影" role="AI 海外产品" date="8 月 20 日" status="待处理" /></div></div>
  </div>;
}

function StatCard({ value, label, tone }: { value: string; label: string; tone?: string }) { return <div className={`stat-card ${tone ?? ''}`}><strong>{value}</strong><span>{label}</span></div>; }
